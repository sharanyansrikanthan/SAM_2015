...
#define SHAR_REMOTE_THRESH 2700
...
{
        printk(" %d CPU \n Instructions : %lu \n Cycles %lu \n Unhalted Ref %lu \n REMOTE DRAM %lu \n LLC MISSES L3hit %lu \n LLC MISSES L2miss %lu \n LLC MISSES L3miss %lu \n LLC REMOTE FWD %lu \n LLC REMOTE HITM %lu \n LLC MISSES %lu \n ROUND COLLECTION %d COLLECTION ACTIVE %d ID %d \n UNHALTED1 %ld UNHALTED2 %ld \n ",cpu, tsk->sure.inst, tsk->sure.cycles, tsk->sure.unhalted, tsk->sure.remote_dram, tsk->sure.llc_l3hit, tsk->sure.llc_l2miss, tsk->sure.llc_l3miss, tsk->sure.llc_remotefwd, tsk->sure.llc_remotehitm, tsk->sure.llc_misses, tsk->round_collection, tsk->collection_active,tsk->pid,tsk->sure.unhalted1,tsk->sure.unhalted2);
        return;
}
	...
	shar_reset_ps_data(&temp);
		...
		if (tsk->pid > shar_id && tsk->collection_active!=0)
		{
                	...
                	while (tsk->round_collection == 3)
			{
				...
				if (rep > 100)
				{
					printk("Waited enough for %d in CPU %d \n",tsk->pid,cpu);
					return -1;
				}
			}
		 	// MEM USAGE
			temp_soc = (long)(SHAR_CYCLES*((float)tsk->sure.llc_misses/(tsk->sure.unhalted1+1)));
			...
			// REMOTE_DRAM
			temp_soc = (long)(SHAR_CYCLES*((float)tsk->sure.remote_dram/(tsk->sure.unhalted1+1)));
			if (temp_soc > SHAR_REMOTE_THRESH)
			{
				...
				printk("\n Remote DRAM accesses from %d %d\n",tsk->pid, tsk->remote_mig);
			}
			...
			// SNOOP IN SOC
			temp_soc = (long)SHAR_CYCLES*(((float)tsk->sure.llc_l2miss/(tsk->sure.unhalted1+1))-(((float)(tsk->sure.llc_l3hit/(tsk->sure.unhalted1+1)))+((float)tsk->sure.llc_l3miss/(tsk->sure.unhalted2+1))));
			...
			// CROSS SOC
			temp_soc = (long)(SHAR_CYCLES*((tsk->sure.llc_remotefwd + tsk->sure.llc_remotehitm)/(float)(tsk->sure.unhalted2+1)));
			if (temp_soc > SHAR_COHERENCE_THRESH/SHAR_PROCESSORS_CORE)
			{
...
static void decide_processes()
{
	...
	int iter = 0, source = 0, destination = 0, iterd = 0, do_nothing = 0, move_done = 0;
	...
	for (counti = 0; counti < SHAR_NUM_SOCKETS && do_nothing == 0; counti++)
	{
					...
					for (iter = 0; iter < SHAR_PROCESSORS_CORE; iter++)
					{
						...
						if (shar_pc_data[source].coherence == 1 && shar_pc_data[source].active == 1)
						{
							...
							{								
								...
								if (shar_pc_data[destination].active == 0 && destination!=no_mov)
								{
									...
									shar_pc_data[destination].snoop = 1;
							...
							if(move_done==0)
							{
								...
								{								
									destination = (iterd*SHAR_NUM_SOCKETS)+counti;
									if (shar_pc_data[destination].snoop == 0 && shar_pc_data[destination].coherence == 0 && shar_pc_data[destination].mem == 0 && destination!=no_mov)
									{
										//shar_ps_data[countj].coherence = 0;
										//shar_ps_data[counti].coherence = 0;
										shar_pc_data[destination].snoop = 1;
								...
								if(move_done==0)
								{
									...
									{								
										destination = (iterd*SHAR_NUM_SOCKETS)+counti;
										if (shar_pc_data[destination].snoop == 0 && shar_pc_data[destination].coherence == 0 && destination!=no_mov && (shar_pc_data[destination].remote_mig != counti+1))
										{
											//shar_ps_data[countj].coherence = 0;
											//shar_ps_data[counti].coherence = 0;
											shar_pc_data[destination].snoop = 1;
		...
		if (do_nothing == 1)
		{
			printk("Returning after swap mem\n");
			return;
		}
		// MEMORY
		flag = 0;
		if (shar_ps_data[counti].mem == 1 && shar_ps_data[counti].coherence == 0 && shar_ps_data[counti].snoop == 0)
		{
			...
			{
				if ((shar_ps_data[countj].mem == 0 && countj!=counti) || ((shar_ps_data[countj].p_mem+1) < shar_ps_data[counti].p_mem))
				{
					if (!(shar_ps_data[countj].mem == 0 && countj!=counti))
						flag = (shar_ps_data[counti].p_mem - shar_ps_data[countj].p_mem)/2;
					for (iter = 0; iter < SHAR_PROCESSORS_CORE && flag > 0; iter++)
					{
						source = (iter*SHAR_NUM_SOCKETS)+counti;
						if (shar_pc_data[source].mem == 1 && shar_pc_data[source].active == 1 && shar_pc_data[source].remote_mig==0)
						{
							iterd = 0;
							move_done = 0;
							while(iterd < SHAR_PROCESSORS_CORE && move_done == 0)
