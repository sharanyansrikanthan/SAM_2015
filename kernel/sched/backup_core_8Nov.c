...
#define SHAR_NUM_SOCKETS 2
#define SHAR_PROCESSORS_CORE 10
#define SHAR_MEM_THRESH 75000000
#define SHAR_COHERENCE_THRESH 600000
...
struct shar_per_socket shar_ps_data[SHAR_NUM_SOCKETS];
...
struct shar_per_core shar_pc_data[SHAR_PROCESSORS_CORE*SHAR_NUM_SOCKETS];
void print_task_collection(struct task_struct *tsk, int cpu)
{
        printk(" %d CPU \n Instructions : %lu \n Cycles %lu \n Unhalted Ref %lu \n LLC MISSES L3hit %lu \n LLC MISSES L2miss %lu \n LLC MISSES L3miss %lu \n LLC REMOTE FWD %lu \n LLC REMOTE HITM %lu \n LLC MISSES %lu \n ROUND COLLECTION %d COLLECTION ACTIVE %d ID %d \n UNHALTED1 %ld UNHALTED2 %ld \n ",cpu, tsk->sure.inst, tsk->sure.cycles, tsk->sure.unhalted, tsk->sure.llc_l3hit, tsk->sure.llc_l2miss, tsk->sure.llc_l3miss, tsk->sure.llc_remotefwd, tsk->sure.llc_remotehitm, tsk->sure.llc_misses, tsk->round_collection, tsk->collection_active,tsk->pid,tsk->sure.unhalted1,tsk->sure.unhalted2);
        return;
}
void print_shar_ps_data(struct shar_per_socket* s)
{
	printk(" %d Snum\n Total Memory  %ld \n Total Cross Socket Snoop %ld \n Total Snoop Inside Socket %ld \n Total Active Processes %d \n DNEW %d \n",s->snum, s->tot_mem, s->tot_crosssoc, s->tot_snoopsoc, s->active_processes, s->dnew);
	return;
}
...
asmlinkage long sys_shar_prog (int num, int data1, int data2)
{
	...
	if (num == 2) // printing data collected
	{
		cpu = data1;
...
static int check_socket_ready (const int snum)
{
	...
	shar_reset_ps_data(&temp);
		...
		if (tsk->pid > shar_id)
		{
			...
			// MEM USAGE
			temp_soc = (long)(SHAR_CYCLES*((float)tsk->sure.llc_misses/tsk->sure.unhalted1));
			...
			// SNOOP IN SOC
			temp_soc = (long)SHAR_CYCLES*(((float)tsk->sure.llc_l2miss/tsk->sure.unhalted1)-((float)(tsk->sure.llc_l3hit/tsk->sure.unhalted1)+((float)tsk->sure.llc_l3miss/tsk->sure.unhalted2)));
			if (temp_soc > SHAR_COHERENCE_THRESH/SHAR_PROCESSORS_CORE)
			{
			...
			// CROSS SOC
			temp_soc = (long)(SHAR_CYCLES*((tsk->sure.llc_remotefwd + tsk->sure.llc_remotehitm)/(float)tsk->sure.unhalted2));
			if (temp_soc > SHAR_COHERENCE_THRESH/SHAR_PROCESSORS_CORE)
			{
...
static void move_to_cpu(const int cpu, struct task_struct *tsk)
{
	...
	//printk("Mask created %d %d\n",cpu1,shar_task); 
	cpumask_clear(&shar_mask);
	//printk("Mask cleared %d %d\n",cpu1,shar_task); 
	cpumask_set_cpu((unsigned int)cpu, &shar_mask);
	//printk("Mask set %d %d\n",cpu1,shar_task); 
	sched_setaffinity(tsk->pid, &shar_mask);
	return;
}
static void swap_processes(const int cpu1,const int cpu2)
{
	struct task_struct *tsk1, *tsk2;
	struct rq *rq1, *rq2;
	int snum = cpu1 % SHAR_NUM_SOCKETS;
	int counti = 0, countj = 0, flag = 0;
	int iter = 0, source = 0, destination = 0, iterd = 0, do_nothing = 0;
	...
	if (tsk1->pid > shar_id)
		...
		if (cpu1 < 2*SHAR_NUM_SOCKETS && cpu1 >= SHAR_NUM_SOCKETS)
		{
			if (shar_ps_data[snum].dnew == 0)
			{
				if (check_socket_ready(cpu1) == 0)
				{
					shar_ps_data[snum].snoop = 0;
					shar_ps_data[snum].coherence = 0;
					shar_ps_data[snum].mem = 0;
					if (shar_ps_data[snum].active_processes > 0 && shar_ps_data[snum].dnew == 1)
					{
					...
					shar_ps_data[snum].dnew = 2;
					reset_measure(cpu1%SHAR_NUM_SOCKETS);				
				}                     
			}	
		}/*
		if (cpu1 == SHAR_NUM_SOCKETS)
		{
			do_nothing = 0;
			for (counti = 0; counti < SHAR_NUM_SOCKETS; counti++)
				if (shar_ps_data[counti].dnew != 2)
					do_nothing = 1;
			for (counti = 0; counti < SHAR_NUM_SOCKETS && do_nothing == 0; counti++)
			{
				...
				{
					iterd = 0;
					for (countj = counti + 1; flag == 0 && countj < SHAR_NUM_SOCKETS; countj++)
					{
						if (shar_ps_data[countj].coherence == 1)
						{
							for (iter = 1; iter < SHAR_PROCESSORS_CORE; iter++)
							{
								source = (iter*SHAR_NUM_SOCKETS)+countj;
								if (shar_pc_data[source].coherence == 1 && shar_pc_data[source].active == 1)
								{
									while(iterd < SHAR_PROCESSORS_CORE)
									{
										...
										{
											shar_ps_data[countj].coherence = 0;
											shar_ps_data[counti].coherence = 0;
											shar_pc_data[destination].active = 1;
											//shar_pc_data[source].active = 0;
											//move_to_cpu(destination,cpu_rq(source)->curr);
											cpumask_clear(&shar_mask);
											//printk("Mask cleared %d %d\n",cpu1,shar_task); 
											cpumask_set_cpu((unsigned int)destination, &shar_mask);
											//printk("Mask set %d %d\n",cpu1,shar_task); 
											sched_setaffinity(cpu_rq(source)->curr->pid, &shar_mask);
										}
				...
				// MEMORY
				flag = 0;
				if (shar_ps_data[counti].mem == 1 && shar_ps_data[counti].coherence == 0 && shar_ps_data[counti].snoop == 0)
				{
					...
					for (countj = 0; flag > 0 && countj < SHAR_NUM_SOCKETS; countj++)
					{
						...
						{
							iterd = 0;
							for (iter = 1; iter < SHAR_PROCESSORS_CORE && flag > 0; iter++)
							{
								source = (iter*SHAR_NUM_SOCKETS)+counti;
								if (shar_pc_data[source].mem == 1 && shar_pc_data[source].active == 1)
								{
									while(iterd < SHAR_PROCESSORS_CORE)
									{
										iterd++;
	...
	shar_enable = 1;/*
	if (tsk1->pid == shar_task)
	{
		printk("CPU number %d %d\n",cpu1,shar_task); 
		
		struct cpumask shar_mask;
		//printk("Mask created %d %d\n",cpu1,shar_task); 
		cpumask_clear(&shar_mask);
		//printk("Mask cleared %d %d\n",cpu1,shar_task); 
		cpumask_set_cpu((unsigned int)cpu2, &shar_mask);
		//printk("Mask set %d %d\n",cpu1,shar_task); 

		sched_setaffinity(shar_task, &shar_mask);
		//shar_task = 9999;
		//migrate_swap(tsk1,tsk2);
	}*/
}
...
static void __sched __schedule(void)
{
	...
	if (cpu > 1 && cpu < 20) {
		...
		{
			shar_swap_count++;
			//if (shar_swap_count <= 10)
			//	printk("Entering the swap routine");	
			swap_processes(cpu,cpu-1);
		}
	}
